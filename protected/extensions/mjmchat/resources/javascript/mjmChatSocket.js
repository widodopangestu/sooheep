$(function () {
    ...
    {
        socket.on('mjmChatMessage', function (username, data, tanggal)
        {
            /* view message */
            $("#mjmChatMessages").append('<li><strong>' + username + ':</strong> ' + data + ' <small class=\'mjmChatDate\'>' + tanggal + '</small></li>');
            mjmChatScrollDown();
        });
        ...
        socket.on('mjmChatUsers', function (users) {
            ...
            {
                var my = '';
                if (value == mjmChatUser)
                    my = ' class=\'mjmChatMyUser\' ';
