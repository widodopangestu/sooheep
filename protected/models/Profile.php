...
class Profile extends CActiveRecord {
    ...
    public function getButtonChat() {
        $tombol = '';
        $cek = Friend::model()->findByAttributes(array('id_user' => Yii::app()->user->id['id'], 'id_user_friend' => $this->id_user));
        $cek2 = Friend::model()->findByAttributes(array('id_user_friend' => Yii::app()->user->id['id'], 'id_user' => $this->id_user));
        if ($cek == null && $cek2 == null) {
            $tombol = '<div style="padding: 5px; background-color: rgb(255, 255, 255); border-radius: 5px;" onclick="window.location=' . CController::createUrl('/m/member/addfriends', array('q' => $this->idUser->hash)) . '"><i class="fa fa-plus" style="color:#f00;"></i></div>';
        } elseif ($cek != null && $cek2 == null) {
            if ($cek->approval == 0)
