...
CREATE TABLE `poll_vote` (
  ...
  CONSTRAINT `vote_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE,
...
ALTER TABLE `sooheeb`.`feeds` 
ADD COLUMN `poll_id` INT(11) NULL COMMENT '' AFTER `repost_id`,
ADD INDEX `feeds_FK_3_idx` (`poll_id` ASC)  COMMENT '';
ALTER TABLE `sooheeb`.`feeds` 
ADD CONSTRAINT `feeds_FK_3`
  FOREIGN KEY (`poll_id`)
  REFERENCES `sooheeb`.`poll` (`id`)
  ON DELETE SET NULL
  ON UPDATE SET NULL;