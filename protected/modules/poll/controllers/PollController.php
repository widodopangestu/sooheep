...
class PollController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow',
                'actions' => array('index', 'view', 'vote', 'open', 'close'),
                'users' => array('@'), //@
            ),
            array('allow',
    ...
    public function actionOpen($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (Yii::app()->user->id['id'] == $model->feed->id_user) {
                $model->status = Poll::STATUS_OPEN;
                $model->save(false);
            } else
                throw new CHttpException(403, 'You don\'t have access.');
    ...
    public function actionClose($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            $model = $this->loadModel($id);
            if (Yii::app()->user->id['id'] == $model->feed->id_user) {
                $model->status = Poll::STATUS_CLOSED;
                $model->save(false);
            } else
                throw new CHttpException(403, 'You don\'t have access.');
